...
function spritelib.setBackgroundDrawingCallback(drawCallback)
	...
	-- put it back in if it was removed
	bgsprite:add()
...
function spritelib:setAnimator(animator, moveWithCollisions, removeOnCollision)
	...
	if animator.startValue ~= nil then
		assert(getmetatable(animator.startValue) == playdate.geometry.point, "playdate.graphics.sprite:setAnimator(): `animator` must use a `playdate.geometry.point` for its start value.")
	end
	if animator.endValue ~= nil then
		assert(getmetatable(animator.endValue) == playdate.geometry.point, "playdate.graphics.sprite:setAnimator()'s `animator` argument must use a `playdate.geometry.point` for its end value.")
	end
